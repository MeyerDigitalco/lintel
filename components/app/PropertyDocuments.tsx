...
export function PropertyDocumentUpload({ propertyId }: { propertyId: string }) {
  ...
  if (!open) {
    return <Button size="sm" onClick={() => setOpen(true)}>Upload document</Button>;
  }
  return (
    <form
      action={async (fd) => {
        await uploadPropertyDocument(fd);
        setOpen(false);
      }}
    ...
    >
      ...
      <input
        ...
        type="file"
        required
        className="block w-full text-sm text-slate file:mr-3 file:rounded-lintel file:border file:border-hairline file:bg-paper file:px-3 file:py-2 file:text-sm sm:col-span-2"
      />
