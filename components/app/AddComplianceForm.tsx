...
export function AddComplianceForm({
  ...
  if (properties.length === 0) {
    return <p className="text-sm text-slate">Add a property first to track compliance.</p>;
  }
  if (!open) {
  ...
  return (
    <Card className="mb-6">
      <CardBody>
        <form action={async (fd) => { await addComplianceItem(fd); setOpen(false); }} className="grid gap-4 sm:grid-cols-2">
          <label className="block">
            <span className="mb-1 block text-sm text-ink">Property</span>
            <select name="property_id" className={inputCls} value={propertyId} onChange={(e) => setPropertyId(e.target.value)}>
          ...
          <label className="block">
            ...
            <span className="mt-1 block text-xs text-slate">Only items required in this property&apos;s nation are shown.</span>
          </label>
          <label className="block">
