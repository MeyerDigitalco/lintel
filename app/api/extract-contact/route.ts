...
import { extractContactCard } from "@/lib/contact-extract";
...
async function authed(req: Request): Promise<boolean> {
  try {
    const u = await createClient().auth.getUser();
    if (u.data.user) return true;
  } catch {
    // no cookie session
  }
...
export async function POST(req: Request) {
  ...
  const fields = await extractContactCard(buffer, file.type || "");
